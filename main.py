...
def main(args, DER_num, lines_num, loads_num, DER_controller, sampling_time = 0.1, disturbance=True):
    # time points
    start = datetime.now()
    x = []
    ...
    if args.mode == 'Vnom':
        x_pre = odeint(DER_controller.VSI_VFctrl_func, x0, t_pre, atol=1e-10, rtol=1e-11, mxstep=5000, printmessg=True)
    else:
        x_pre = odeint(DER_controller.VSI_VFctrl_func, x_critic, t_pre, atol=1e-10, rtol=1e-11, mxstep=5000, printmessg=True)
    ...
    for step in range(20):
        ...
        steps = 11 + step
        if args.mode == 'Vnom':
            x_af = odeint(DER_controller.VSI_VFctrl_func, x1, np.array([(steps - 1) * sampling_time, steps * sampling_time]), atol=1e-10, rtol=1e-11, mxstep=5000, printmessg=True)
            x = x + [x_af[-1].tolist()]
            x1 = (x_af[-1]).tolist()
        else:
            x_af = odeint(DER_controller.VSI_VFctrl_func, x1, np.array([(steps - 1) * sampling_time, steps * sampling_time]), atol=1e-10, rtol=1e-11, mxstep=5000, printmessg=True)
...
if __name__ == '__main__':
    ...
    for i in range(num_test):
        ...
        der_controller = DER_controller(args.mode, args.critic_bus_id, DER_num, lines_num, loads_num, DER_dic, BUSES,
                                        ...
                                        G, Vnom, wref, mp1, rN, wc, F, wb, Lf, Cf, rLf, Lc, rLc, kp, ki,  sampling_time=sampling_time)
        reward = main(args, DER_num, lines_num, loads_num, der_controller, sampling_time, disturbance=False)  # False True
